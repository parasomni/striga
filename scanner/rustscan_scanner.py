...
async def run_rustscan_scan(target):
    ...
    rustscan_flags = config.get_tool_flags(module_name, "scanner")
    ...
    rustscan_results = parse_rustscan_results(stdout)
    logger.log(f"{Fore.LIGHTBLUE_EX}[+]{Style.RESET_ALL} {module_name} scan for {target} finished.")
    return json.dumps(rustscan_results, indent=4)


def parse_rustscan_results(stdout):
    try:
